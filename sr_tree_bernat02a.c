    ...
    *(dist_to_node + epsilon_0);
    ...
    if(theta< theta_max){
      ...
      *(dist_to_node + epsilon_0);
      ...
      if((*node)-> right_up && (*node)-> right_up->tot_mass>0)
        get_acc_on_body(pos_x, pos_y, &(*node)->right_up, theta_max, G);
      if((*node)-> right_down && (*node)-> right_down->tot_mass>0)
        get_acc_on_body(pos_x, pos_y, &(*node)->right_down, theta_max, G);
      if((*node)-> left_up && (*node)-> left_up->tot_mass>0)
        get_acc_on_body(pos_x, pos_y, &(*node)->left_up, theta_max, G);
      if((*node)-> left_down && (*node)-> left_down->tot_mass>0)
