...
int main(int argc, char *args[]){
  ...
  if(file == NULL){ printf("Error while reading file.\n");}
  /*maybe in this case we could allocate memory for this
  matrix statically*/
  double *pos_vel = (double *)malloc(4*N*sizeof(double));
  ...
  //CHECK if new array is ok
  // printf("We read the file\n");
  // int ok =1;
  // for(int i=0; i<N; i+=2){
  //   if(arr[i][0]!=pos_vel[i*4] || arr[i][1]!=pos_vel[4*i+1]){
  //     printf("Error array no es el que toca 1\n");
  //     ok =0;
  //   }
  // }
  // if(ok){
  //   printf("The file has been read and everything seems ok.\n");
  // }
  const int K=200;
  double pow_2[K];
  pow_2[0]=1.0;
  ...
  //  printf("inserting nodes in the tree...\n");
    ...
    for(int i=0; i<N; i++){
      id_tar = i;
      insert(&root ,pos_vel[4*i], pos_vel[4*i+1], ma[i], pow_2, id_tar);
      //insert(&root ,arr[i][POS_X], arr[i][POS_Y], ma[i], pow_2, id_tar);
    }
    for(int i=0; i <N; i ++){
      get_acc_on_body(pos_vel[4*i], pos_vel[4*i+1], &root, theta_max, G);
    //  printf("Acceleration on %d: (%lf , %lf ) \n", i+1, total_acc_x, total_acc_y);
      pos_vel[4*i+2] += delta_t  * total_acc_x;
      pos_vel[4*i+3] += delta_t  * total_acc_y;
      pos_vel[4*i] += delta_t * pos_vel[4*i+2];
      pos_vel[4*i+1] += delta_t * pos_vel[4*i+3];
      total_acc_x=0;
      total_acc_y=0;
    }
    //CHECK if new array is ok
    // for(int i=0; i<N; i+=2){
    //   if(arr[i][0]!=pos_vel[i*4] || arr[i][1]!=pos_vel[4*i+1])
    //     printf("Error array no es el que toca 385\n");
    // }
